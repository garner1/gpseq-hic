...
# create a dataset
data_intra = read.table("/home/garner1/Work/dataset/gpseq+hic/layer-delta-medianCount.intra.symmetric.tsv",header=FALSE,col.names = c('centrality','layer_distance','HiC'))
...
data_inter = read.table("/home/garner1/Work/dataset/gpseq+hic/layer-delta-medianCount.inter.symmetric.tsv",header=FALSE,col.names = c('centrality','layer_distance','HiC'))
...
ggplot(data_intra, aes(fill=layer_distance, y=HiC, x=centrality)) + geom_area(aes(fill = layer_distance, group = layer_distance),position = "stack")+scale_fill_brewer(palette = "Set3")
ggsave(filename = "intra-chrom.hicVScentrality.pdf", device = cairo_pdf)
dev.off()
ggplot(intra_perc, aes(fill=layer_distance, y=percent, x=centrality)) + geom_area(aes(fill = layer_distance, group = layer_distance),position = "stack")+scale_fill_brewer(palette = "Set3")
ggsave(filename = "intra-chrom.hicVScentrality.percentage.pdf", device = cairo_pdf)
dev.off()

ggplot(data_inter, aes(fill=layer_distance, y=HiC, x=centrality)) + geom_area(aes(fill = layer_distance, group = layer_distance),position = "stack")+scale_fill_brewer(palette = "Set3")
ggsave(filename = "inter-chrom.hicVScentrality.pdf", device = cairo_pdf)
dev.off()
ggplot(inter_perc, aes(fill=layer_distance, y=percent, x=centrality)) + geom_area(aes(fill = layer_distance, group = layer_distance),position = "stack")+scale_fill_brewer(palette = "Set3")
